...
public class ElasticsearchClientServiceImpl implements ElasticsearchClientService {
    ...
    public List<Article> fetchArticlesWithMustQuery(ArticleQuery articleQuery) throws IOException {
        ...
        SearchResponse<Article> articleSearchResponse = elasticsearchClient.search(
                req ->
                        req.index(index)
                                .size(articleQuery.getSize())
                                .query(query ->
                                        query.bool(bool ->
                                                bool.must(queries))),
                Article.class);
        return articleSearchResponse.hits().hits().stream()
    ...
    private List<Query> prepareQueryList(ArticleQuery articleQuery) {
        var queryMap = Map.of(
                "category_id", articleQuery.getCategoryId(), "author_id", articleQuery.getAuthorId());

        var queries = queryMap.entrySet()
                .stream()
